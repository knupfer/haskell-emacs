          ...
          (unless (file-directory-p haskell-emacs-dir)
            ...
            (when arg
              (with-temp-buffer
                ...
                (let ((action 'run))
                  (when (yes-or-no-p "Create a cabal sandbox? ")
                    (call-process "cabal" nil nil nil '("sandbox" "init")))
                  (when (yes-or-no-p "Cabal install the dependencies? ")
                    (call-process "cabal" nil nil nil '("install"
                                                        "atto-lisp"
                                                        "utf8-string"
                                                        "text")))
                  (if (yes-or-no-p "Add a simple example? ")
                      (progn (insert "
module Matrix (transpose, dyadic) where
