...
* Examples
Melpa install =haskell-emacs= (if you're clone the repo directly,
then you have to add the repo to your =load-path=, =(require
...
you'll receive the following demo library:
...
which haskell-emacs cannot marshal to the correct type. Please keep in mind that Emacs Lisp Arrays will be
translated (recursively) to Haskell Lists and Emacs Lisp Lists will be marshaled to either Haskell Lists or
Haskell Tuples.
