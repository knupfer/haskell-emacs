...
#+BEGIN_SRC haskell
  ...
  bufferString :: Emacs Text
  bufferString = eval [Symbol "buffer-string"]

  gotoLine :: Int -> Emacs Int
  gotoLine n = eval [Symbol "goto-line", Number $ fromIntegral n]

  notEmpty :: Text -> [Int]
  notEmpty str = [n | (l,n) <- zip (T.lines str) [1..], not $ T.all isSpace l]

  commentLines2 :: Emacs Text
  commentLines2 = do prefix <- toPrefix <$> majorMode
                     ls     <- notEmpty <$> bufferString
...
The first implementation takes 20% more time, even though the second
