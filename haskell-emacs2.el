  ...
  (save-excursion
    (let ((funs (directory-files haskell-emacs-dir nil "^[^.].*\.hs$")))
      (let ((imports)
        ...
        (when haskell-emacs-process2
          (delete-process haskell-emacs-process2))
        (setq haskell-emacs-process2
              (start-process "hask" nil
                             (concat haskell-emacs-dir ".HaskellEmacs")))
        (set-process-query-on-exit-flag haskell-emacs-process2 nil)
        (set-process-filter
         haskell-emacs-process2 (lambda (proc str)
                                  (setq haskell-emacs--response
                                        (concat haskell-emacs--response str))))
        (mapc (lambda (fi)
                (mapc (lambda (fu) (eval (haskell-emacs--wrapper-fun2 fu)))
                      fi))
              exports)))))

(defun haskell-emacs--wrapper-fun2 (fun)
