...
allExports =
  ...
  where
    ...
    coerceMdlNameLoc :: ModuleName SrcSpanInfo -> ModuleName SrcLoc
    coerceMdlNameLoc (ModuleName _ nm) = ModuleName noLoc nm
    coerceMdlNameUnit :: ModuleName SrcSpanInfo -> ModuleName ()
    coerceMdlNameUnit (ModuleName _ nm) = ModuleName () nm
    coerceNameUnit :: Name SrcSpanInfo -> Name ()
      ...
      case mdl of
        (Module _ mMdlHead _ _ decls) ->
          case mMdlHead of
            ...
            Just mdlHead -> Right $
              (nameFromModuleHead mdlHead,
               fromJust (extractExportsFromHeader mdlHead <|> Just (exportsFromDecls decls)))
        XmlPage _ mdlName _ _ _ _ _ ->
          Left $ "TODO: Error parsing exports for XmlPage: " <> fromModuleName mdlName
        XmlHybrid _ mdlHead _ _ _ _ _ _ _ ->
          Left $ "TODO: Error parsing exports for XmlHybrid: " <> show (fromModuleHead <$> mdlHead)
    ParseFailed _ msg -> Left msg
  where
    parseSrc :: ParseResult (Module SrcSpanInfo)
    parseSrc = parseFileContentsWithMode
                     defaultParseMode {fixities = Nothing}
                     content
    extractExportsFromHeader :: ModuleHead SrcSpanInfo -> Maybe [Name SrcSpanInfo]
...
fromName :: Name l -> String
fromName (S.Symbol _ str) = str
fromName (S.Ident  _ str) = str

fromModuleName :: ModuleName l -> String
fromModuleName (ModuleName _ str) = str

nameFromModuleHead :: ModuleHead l -> ModuleName l
nameFromModuleHead (ModuleHead _ mname _ _) = mname

fromModuleHead :: ModuleHead l -> String
fromModuleHead = fromModuleName . nameFromModuleHead
